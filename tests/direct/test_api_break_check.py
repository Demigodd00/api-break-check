...
def test_complete_map_and_validator_replay(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    direct_vm.mock_llm(PROMPT, json.dumps({"classification": "COMPATIBLE", "migration_note": "Existing required fields and meanings remain unchanged for the registered client."}))
    contract.assess_endpoint("profile")
    leader = direct_vm._captured_validators[-1][0]
    direct_vm.clear_mocks()
    direct_vm.mock_llm(PROMPT, json.dumps({"classification": "COMPATIBLE", "migration_note": "The optional addition leaves every registered consumer usage intact without adaptation."}))
    assert direct_vm.run_validator(leader_result=leader) is True
    direct_vm.clear_mocks()
    direct_vm.mock_llm(PROMPT, json.dumps({"classification": "BREAKING", "migration_note": "This intentionally different decision must be rejected even though its note is well formed."}))
...
def test_flagged_revision_and_bad_model_output_fail_closed(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = deploy(direct_vm, direct_deploy, direct_alice)
    prepare(contract, direct_vm, direct_alice, direct_bob)
    direct_vm.mock_llm(PROMPT, json.dumps({"classification": "BREAKING", "migration_note": "The proposed response removes a required field used by the registered client."}))
    contract.assess_endpoint("profile")
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(PROMPT, json.dumps({"classification": "UNKNOWN", "migration_note": "This category is outside the closed response schema."}))
    with direct_vm.expect_revert("invalid_classification"):
        contract.assess_endpoint("profile")
